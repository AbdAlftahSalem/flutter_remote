/**
 * Flutter Remote WebRTC V3 KeyframeController
 *
 * Owns:
 *   - SPS, PPS, and IDR keyframe caching
 *   - IDR recovery state machine (NORMAL, RECOVERY_REQUESTED, WAITING_FOR_IDR, IDR_RECEIVED, ERROR)
 *   - Fresh keyframe request coalescing
 *   - Replacement FFmpeg encoder coordination to maintain reference chain
 *   - Keyframe recovery latency and success metrics
 */

import { EventEmitter } from 'node:events';
import { NAL_TYPES } from './RtpPacketizer.js';
...
export class KeyframeController extends EventEmitter {
  constructor(options = {}) {
    super();
  ...
  getKeyframePackets(fps = 30) {
    ...
    // Extract IDR from cachedKeyframe
    let buffer = this.cachedKeyframe;
    ...
    return new Promise((resolve, reject) => {
      ...
      const onRecoverySuccess = (auNals) => {
        ...
        try {
          ...
          if (!hasSps || !hasPps) {
            throw new Error(`Recovery IDR missing SPS/PPS. SPS=${hasSps} PPS=${hasPps}`);
          }

          // Cache SPS, PPS, IDR
          this.inspectAndCacheNals(auNals);

          // Packetize IDR Access Unit
          ...
          const ssrc = this.rtpPacketizer.ssrc;
